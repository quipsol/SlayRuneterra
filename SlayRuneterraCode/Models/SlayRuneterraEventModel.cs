...
public abstract class SlayRuneterraEventModel : CustomEventModel
{
    ...
    protected EventOption AllowOrLockOption(Func<bool> evaluate, Func<Task>? onChosen, IEnumerable<IHoverTip> tips, string pageKey = "INITIAL")
    {
        ...
        if (evaluate.Invoke())
            return new EventOption(this, onChosen, $"{this.Id.Entry}.pages.{pageKey}.options.{StringHelper.Slugify(optionKey)}", tips);
        optionKey += "_LOCKED";
        return EmptyOption(StringHelper.Slugify(optionKey), pageKey);
